/* =========================================
   ...
   MANA STRENGTH — SET BY SET LOGGER
   ========================================= */

(() => {
  "use strict";

  const STYLE_ID = "mana-strength-v64-style";
  const SCREEN_ID = "manaStrengthV64Workout";

  const PROGRAM_KEY = "mana-strength-v62-program";
  const OLD_LOG_KEY = "mana-strength-v63-logs";
  ...
  function latestExercise(name) {
    ...
    /*
      Fallback to v6.3 data so previous
      single-set entries aren't lost.
    */
    const oldLogs =
      safeJson(
        localStorage.getItem(OLD_LOG_KEY) || "[]",
        []
      );

    for (
      let i = oldLogs.length - 1;
      i >= 0;
      i--
    ) {
      const old =
  ...
  function ensureScreen() {
    ...
    document
      .getElementById("manaV64Close")
      .onclick = closeWorkout;

    document
      .getElementById("manaV64Complete")
  ...
  function previousText(previous) {
    if (
      !previous ||
      !Array.isArray(previous.sets) ||
      !previous.sets.length
    ) {
      return "No previous set history";
    }
  ...
  function createSetRow(
    ...
    row.className = "mana-v64-set";
    row.dataset.v64Set = "1";
  ...
  function buildExerciseCard(
    ...
    const target = exercise[1];

    const previous =
      latestExercise(name);

    const setCount =
      parseSetCount(target);

    const card =
      document.createElement("div");

    card.className = "mana-v64-card";
    ...
    for (
      ...
      list.appendChild(
        createSetRow(
          i + 1,
          previous?.sets?.[i] || null
        )
      );
    }

    card
      .querySelector("[data-v64-add]")
      .onclick = () => {
        const count =
  ...
  /*
    v6.4 takes control of the existing
    v6.3 Start workout buttons without
    changing v6.3 itself.
  */
